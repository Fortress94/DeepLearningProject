...
def train(model, filename):
    ...
    while iteration < model.number_of_iterations:
        ...
        if iteration % 500 == 0:
            outputlog.append('%s;%s;%s;%s;%s;\n' % (iteration, epsilon, qmax, loss.item(), reward_counter))
            writer.add_scalar('data/epsilon', epsilon, iteration)
            writer.add_scalar('data/loss', loss.item(), iteration)
            writer.add_scalar('data/q-max', qmax, iteration)
            writer.add_scalar('data/reward', reward_counter, iteration)
